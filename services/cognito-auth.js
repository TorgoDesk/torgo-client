...
import { UniversalStorageWrapper } from "~/services/UniversalStorageWrapper.js";


export default class CognitoAuthScheme {
  constructor(auth, options, router) {
    this.$auth = auth;
    this.$router = router;
  ...
  async login({ data }) {
    ...
    const result = await this._login(data.username, data.password, data.newPassword);
    if (result.newPasswordRequired) {
      console.debug("New password required. Should redirect to confirm page")
      return result
    }
  ...
  async _login(Username, Password, NewPassword) {
